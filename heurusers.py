...
# this is necessary because userIDs are generated by iterating through a dict, and are therefore arbitrary
del users, addresses, usedAddresses, inputTxs, outputTxs, usersDict
print "deletions executed"
userIDs = dict()
with open("bitcoinData/heurusers.csv", "r") as userFile:
...
outputTuples = [[]]
# store association of outputs' user and values in a list, the index being the txID
with open("bitcoinData/newOutputs.csv", "r") as outputs:
    for line in outputs:
        data = parseOutput(line)
        txID, value, address = int(data[0]), data[3], data[5]
        while len(outputTuples) <= txID:
            outputTuples.append([])
        if len(outputTuples[txID]) == 0:
            outputTuples[txID] = [(userIDs[address], value)]
        else:
            outputTuples[txID].append((userIDs[address], value))

with open("bitcoinData/heurTxs.csv", "w") as heurTxs, open("bitcoinData/txs.csv", "r") as txs:
